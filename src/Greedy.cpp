...
int select_origin_stack(Layout& layout, int s_d, int blocked, multimap<int, int> gv2index){
    deque<int>& ss_d = layout.stacks[s_d];
    ...
    for (int s_o=0; s_o < layout.size(); s_o++){
            ...
            deque<int>& ss_o = layout.stacks[s_o];
...
void get_sorted_gvalues(deque<int> stack, multimap <int ,int>& gv2index){
